...
dims_early = dims(1:5000,:);%first 2000 trials
dims_late = dims(end-4999:end,:);%first 2000 trials
...
subplot 122
tapNum = watershed_sandbox(dims_all,11)';
title('Watershed')
%subplot 122
%tapNum_late = watershed_sandbox(dims_late,8)';
%title('Watershed Late')
N = max(tapNum);
...
tapNum_early = tapNum(1:5000);
tapNum_late = tapNum(5001:end);
...
%tapNum_early = tapNum(1:2000);
% have to assign each tap to a bin
% T is N^2

T_early = zeros(N);
R_early = zeros(N);
rewards_early = rewards(1:5000);
...
R_early=R_early./T_early; % normalize by frequency

T_early(T_early<0.02*length(tapNum_early)/N)=0;
R_early(T_early==0)=0;
%T_early = T_early./sum(T_early(:)); %proportion transition matrix
%R_early = R_early./sum(R_early(:)); %proportion transition matrix
...
c = colorbar;
c.Label.String = 'Avg Reward Rate';
title('Reward Matrix (Early)')
prettyplot(12)
...
%         R2(tapNum_early2(i), tapNum_early2(i+1),p) = R2(tapNum_early2(i), tapNum_early2(i+1))+rewards(i);%rewards for that transition
% 
%     end
%     R2(:,:,p)=R2(:,:,p)./T2(:,:,p); % normalize by frequency
%     %T2(:,:,p) = T2(:,:,p)./sumCts;
% end
% 
% % taps
...
%tapNum_late = tapNum(2001:end);
% have to assign each tap to a bin
% T is N^2=

T_late = zeros(N);
R_late = zeros(N);
rewards_late = rewards(end-4999:end);
...
R_late=R_late./T_late; % normalize by frequency

T_late(T_late<0.02*length(tapNum_late)/N)=0;
R_late(T_late==0)=0;
T_late = T_late./sum(T_late(:)); %proportion transition matrix
%R_late = R_late./sum(R_late(:)); %proportion transition matrix
...
% figure;
% subplot 121;
% scatter(reshape(log(TTT_early),[1 N^2]),reshape(log(RRR_early),[1 N^2]))
% [r,p]=corrcoef(reshape(TTT_early,[1 N^2]),reshape(RRR_early,[1 N^2]),'row','complete')
...
% 
% subplot 122;
% scatter(reshape(log(TTT_late),[1 N^2]),reshape(log(RRR_late),[1 N^2]))
% [r,p]=corrcoef(reshape(TTT_late,[1 N^2]),reshape(RRR_late,[1 N^2]),'row','complete')
% text(1.3,.5,strcat('r^2:',num2str(r(1,2)^2)))
% text(1.3,.4,strcat('p:',num2str(p(1,2))))
% axis square;
% xlabel('transition matrix values')
...
% title('Reward Matrix')
% 
% subplot 243;scatter(reshape(T_early,[1 N^2]),reshape(R_early,[1 N^2]));axis square;
% xlabel('transitions')
% ylabel('rewards')
% 
...
% subplot 247;scatter(reshape(T_late,[1 N^2]),reshape(R_late,[1 N^2]));axis square; 
% xlabel('transitions')
% ylabel('rewards')
% 
